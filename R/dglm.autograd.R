...
#' 
#' Fits a univariate Gaussian spatial regression model: \eqn{\log(\mu)=x^T\beta} and \eqn{\log(\phi)=z^T\gamma}. Parameters not listed below are optional.
#' 
#' @param y observed response 
#' @param x covariates for the mean model
#' @param z covariates for the dispersion model
#' @param lower.xi lower bound for the index parameter of the Tweedie CP-g distribution
#' @param upper.xi upper bound for the index parameter of the Tweedie CP-g distribution
#' 
####################################
...
dglm.autograd <- function(y = NULL,
  ...
  ztz = crossprod(z, z)
  A.gamma.mat = ztz + (1e6 + prec.gamma) * diag(q)
  ...
  for(i in 1:niter){
    ...
    A.beta.mat = forceSymmetric(crossprod(x * as.vector(t1), x) + (1e6 + prec.beta) * diag(p))
    ...
    trgt_dfn[i] = sum(log(lik.val)) + 
      mvtnorm::dmvnorm(as.vector(beta), rep(0, p), diag(p)/prec.beta, log = T) + 
      mvtnorm::dmvnorm(as.vector(gamma), rep(0, q), diag(q)/prec.beta, log = T) + 0
...
# ## Not Run
# setwd("/home/gxk9jg/scripts/samplers/autograd/")
# source("dglm.autograd.R")
# source("delphi.r")
# source("../plot_mcmc.R")
# set.seed(2022)
# require(tweedie)
...
# 
# # weights = rgamma(N, shape = 1, scale = 1)
# 
# xi.true = 1.5
# 
# y_sim = rtweedie(N, xi = xi.true, mu = mu_sim, phi = phi_sim); range(y_sim)
...
# lower.xi = 1
# upper.xi = 2
# system.time(mc <- dglm.autograd(y=y, x=x, z=z, lower.xi = lower.xi, upper.xi = upper.xi, niter = niter, verbose=T, thin = 20))
# # model summary
...
# par(mfcol=c(1,1))
# plot(as.ts(mc$target_fn), ylab = "Target Fn."); grid();
# abline(h = sum(log(dtweedie(y = y, xi = xi.true, mu = mu_sim/weights, phi = phi_sim/weights^(2 -xi.true))))+
#          mvtnorm::dmvnorm(as.vector(beta.true), rep(0,p),diag(p)/prec.beta,log=T)+
#          mvtnorm::dmvnorm(as.vector(gamma.true), rep(0,q),diag(q)/prec.beta,log=T), col="red")
# plot_mcmc(mc$beta.mcmc,true = beta.true)
# plot_mcmc(mc$gamma.mcmc,true = gamma.true)
# plot_mcmc(mc$xi.mcmc, true =  xi.true, cnames= "xi")
# #true vs estimated
# par(mfcol = c(1,2))
# plot(exp(x%*%mc$model[1:p,"median"]), mu_sim, xlab = "Estimated Mean", ylab = "True Mean"); abline(c(0,1),col="blue"); grid()
# plot(exp(z%*%mc$model[(p+1):(p+q),"median"]), phi_sim, xlab = "Estimated Dispersion", ylab = "True Dispersion"); abline(c(0,1),col="blue"); grid()
